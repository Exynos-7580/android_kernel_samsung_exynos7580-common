...
static void *zs_zpool_create(char *name, gfp_t gfp, struct zpool_ops *zpool_ops)
{
	 ...
	 * gfp mask.
	 */
	return zs_create_pool(name, &zs_zpool_ops);
}
...
static int reclaim_zspage(struct zs_pool *pool, struct page *first_page)
{
	...
	while (page) {
		...
		while ((offset = obj_idx_to_offset(page, idx, class->size))
					< PAGE_SIZE) {
			handle = (unsigned long)location_to_obj(page, idx++);
...
static void obj_zs_free(struct zs_pool *pool, struct size_class *class,
			unsigned long obj)
{
	struct link_free *link;
				...
				struct zs_compact_control *cc)
{
	...
	while (1) {
		...
		unpin_tag(handle);
		obj_zs_free(pool, class, used_obj);
	}
...
static struct page *isolate_source_page(struct size_class *class)
{
	...
	if (page)
		remove_zspage(class, ZS_ALMOST_EMPTY, page);
